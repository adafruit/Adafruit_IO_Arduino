 ...
 *
 * In addition, this example allows you to enter your Adafruit IO username and
 * key as customer parameters in WiFiManager so that they do not need to be
 * coded into the sketch.
 *
 * This is useful if you want to create projects and share them with others that
 * may use them on a different WiFi network and use a different Adafruit IO
 * account for IOT integrations such as collecting sensor data or voice command
 * integration via IFFT.
 *
 * To use this example, setup a feed called "myfeed".  When the ESP8266 or ESP32
 * microcontroller starts, join the "WiFi Setup" SSID and you should be
 * presented with the config portal.  If the config portal does not
 * automatically start you can browse to http://192.168.4.1 to access it
 *
 * Select the SSID and enter the password for WiFi Access in the config portal.
 * Enter your Adafruit IO username and key in the config portal and select
 * "Save".
 *
...
WiFiManagerParameter custom_IO_USERNAME("iouser", "Adafruit IO Username",
                                        IO_USERNAME, 60);
...

{
  ...
  wifiManager.addParameter(
      &custom_IO_USERNAME); // set custom paraeter for IO username
  ...
  wifiManager.setSaveConfigCallback(
      saveConfigCallback); // set config save notify callback
